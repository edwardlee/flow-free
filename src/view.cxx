          ...
          solved_ui_("Solved_UI.png")
{
    ...
    for(int i = red; i <= white; ++i) {
        ...
        horiz_conns_.push_back(ge211::Rectangle_sprite({grid_size,
                                                        grid_size / 4},
                                                       colors[i]));
...
View::draw(Sprite_set &set)
{
    ...
    /// Drawing the endpoints.
    for (int i = 0; i < model_.dims().width; i++) {
        for (int j = 0; j < model_.dims().height; j++) {
            if(model_.endpts[i][j])
                set.add_sprite(endpts_[model_.endpts[i][j] - 1],
                               mid_bts({i, j}),
                               2);
        }
    }

    /// Drawing the connections between lines.
    for (int i = 0; i < (int)model_.vert_conns.size(); i++) {
        for (int j = 0; j < (int)model_.vert_conns[0].size(); j++) {
            if (model_.vert_conns[i][j] != 0) {
                size_t c = model_.vert_conns[i][j] - 1;
                set.add_sprite(vert_conns_[c],
                               vert_conn_mid_bts({i, j}),
                               1);
                set.add_sprite(corners_[c],
                               vert_conn_mid_bts({i, j}).up_by(grid_size/8),
                               1);
                set.add_sprite(corners_[c],
                               vert_conn_mid_bts({i, j+1}).up_by(grid_size/8),
                               1);
            }
        }
    }
    for (int i = 0; i < (int)model_.horiz_conns.size(); ++i) {
        for (int j = 0; j < (int)model_.horiz_conns[0].size(); ++j) {
            if (model_.horiz_conns[i][j] != 0) {
                ...
                set.add_sprite(horiz_conns_[c],
                               horiz_conn_mid_bts({i, j}),
                               1);
                set.add_sprite(corners_[c],
                               horiz_conn_mid_bts({i, j}).left_by(grid_size/8),
                               1);
                set.add_sprite(corners_[c],
                               horiz_conn_mid_bts({i+1, j}).left_by(grid_size/8),
                               1);
            }
        }
    }
    ...
    /// Drawing the UI for when the game is solved.
    if (model_.solved()) {
        set.add_sprite(solved_ui_,
                       {(model_.dims().width - 6) * grid_size / 2,
                           (model_.dims().width - 6) * grid_size / 2},
                       3);
    }
}
